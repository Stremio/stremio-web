...
const mapSelectableInputs = (library) => {
  const typeSelect = {
    title: "Select type",
    options: library.selectable.types.map(({ type, deepLinks }) => ({
  ...
  const sortSelect = {
    title: "Select sort",
    options: library.selectable.sorts.map(({ sort, deepLinks }) => ({
      value: deepLinks.library,
      label: sort,
    })),
  ...
  const paginationInput =
    library.selectable.prevPage || library.selectable.nextPage
      ? {
