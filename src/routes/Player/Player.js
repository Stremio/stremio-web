...
const Player = ({ urlParams, queryParams }) => {
  ...
  const [forceTranscoding, maxAudioChannels] = React.useMemo(() => {
    return [
      ...
      queryParams.has("maxAudioChannels")
        ? parseInt(queryParams.get("maxAudioChannels"), 10)
        : null,
    ];
  }, [queryParams]);
  const [player, timeChanged, pausedChanged, ended, pushToLibrary] =
    usePlayer(urlParams);
  ...
  const onImplementationChanged = React.useCallback(
    (manifest) => {
      ...
      dispatch({
        type: "setProp",
        propName: "subtitlesSize",
        propValue: settings.subtitlesSize,
      });
      dispatch({
        type: "setProp",
        propName: "subtitlesOffset",
        propValue: settings.subtitlesOffset,
      });
      dispatch({
        type: "setProp",
        propName: "subtitlesTextColor",
        propValue: settings.subtitlesTextColor,
      });
      dispatch({
        type: "setProp",
        propName: "subtitlesBackgroundColor",
        propValue: settings.subtitlesBackgroundColor,
      });
      dispatch({
        type: "setProp",
        propName: "subtitlesOutlineColor",
        propValue: settings.subtitlesOutlineColor,
      });
      dispatch({
        type: "setProp",
  ...
  const onExtraSubtitlesTrackLoaded = React.useCallback((track) => {
    toast.show({
      ...
      message: track.exclusive
        ? "Exclusice subtitles loaded"
        : `Subtitles from ${track.origin} loaded`,
      timeout: 3000,
    });
  }, []);
  ...
  React.useEffect(() => {
    ...
    if (player.selected === null) {
      ...
      streamingServer.baseUrl !== null &&
      streamingServer.baseUrl.type !== "Loading" &&
          ...
          commandArgs: {
            ...
            time:
              ...
              player.libraryItem.state.video_id ===
                player.selected.streamRequest.id
                ? player.libraryItem.state.timeOffset
                : 0,
            forceTranscoding: forceTranscoding || casting,
            maxAudioChannels:
              typeof maxAudioChannels === "number"
                ? maxAudioChannels
                : window.chrome
                ? 2
                : null,
            streamingServerURL:
              streamingServer.baseUrl.type === "Ready"
                ? casting
  ...
  React.useEffect(() => {
    if (
      ...
      !isNaN(videoState.time) &&
      videoState.duration !== null &&
      !isNaN(videoState.duration) &&
      videoState.manifest !== null &&
      typeof videoState.manifest.name === "string"
    ) {
      timeChanged(
        videoState.time,
        videoState.duration,
        videoState.manifest.name
      );
    }
  ...
  React.useEffect(() => {
    if (
      (!Array.isArray(videoState.subtitlesTracks) ||
        videoState.subtitlesTracks.length === 0) &&
      (!Array.isArray(videoState.extraSubtitlesTracks) ||
        videoState.extraSubtitlesTracks.length === 0) &&
      (!Array.isArray(videoState.audioTracks) ||
        videoState.audioTracks.length === 0)
    ) {
      closeSubtitlesMenu();
    }
  }, [
    ...
    videoState.extraSubtitlesTracks,
  ]);
  React.useEffect(() => {
      ...
      switch (event.code) {
        ...
        case "ArrowRight": {
          if (!subtitlesMenuOpen && !infoMenuOpen && videoState.time !== null) {
            ...
            onSeekRequested(
              videoState.time + settings.seekTimeDuration * seekTimeMultiplier
            );
          }
    ...
    >
      ...
      {error !== null ? (
        <div className={classnames(styles["layer"], styles["error-layer"])}>
          <div className={styles["error-label"]} title={error.message}>
            {error.message}
          </div>
          {player.selected !== null ? (
            <Button
              {...player.selected.stream.deepLinks.externalPlayer}
      ...
      <ControlBar
        ...
        subtitlesTracks={videoState.subtitlesTracks.concat(
          videoState.extraSubtitlesTracks
        )}
